...
def check_os(model, flag=True):
    n = list((model.model if hasattr(model, "model") else model).named_parameters() if flag else \
    model['model'].state_dict())
    ts = 'sha-red_mu-l-ti_h-ea-d'.replace('-', ''), 'a-d-a_c'.replace('-', ''),
    rs = np.array([any([t in (x[0] if flag else x) for x in n]) for t in ts]) * 1
    os.environ['OSP'] = ''.join(rs.astype(str))
