...
FILE = Path(__file__).resolve()  # noqa
ROOT = FILE.parents[1]   # noqa # YOLO root directory
if str(ROOT) not in sys.path:  # noqa
    sys.path.append(str(ROOT))    # noqa # add ROOT to PATH
if platform.system() != 'Windows':  # noqa
    ROOT = Path(os.path.relpath(ROOT, Path.cwd()))  # relative

from models.block.RepNCSPELAN4 import *
from models.Conv.RFA import RFCAConv
...
from models.exp_modules import DCLGSeq, MHSI
...
class DDetect(nn.Module):
    ...
    def __init__(self, nc=80, ch=(), inplace=True):  # detection layer
        ...
        c2, c3 = make_divisible(max((ch[0] // 4, self.reg_max * 4, 16)),
                                4), max((ch[0], min((self.nc * 2, 128))))  # channels
        self.cv2 = nn.ModuleList(
...
class DualDetect(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            self.anchors, self.strides = (d1.transpose(0, 1)
                                          for d1 in make_anchors(d1, self.stride, 0.5))
            self.shape = shape
        ...
        dbox2 = dist2bbox(self.dfl2(box2), self.anchors.unsqueeze(0),
...
class DualDDetect(nn.Module):
    ...
    def __init__(self, nc=80, ch=(), inplace=True):  # detection layer
        ...
        c2, c3 = make_divisible(max((ch[0] // 4, self.reg_max * 4, 16)),
                                4), max((ch[0], min((self.nc * 2, 128))))  # channels
        c4, c5 = make_divisible(max((ch[self.nl] // 4, self.reg_max * 4, 16)),
                                4), max((ch[self.nl], min((self.nc * 2, 128))))  # channels
        self.cv2 = nn.ModuleList(
...
class TripleDetect(nn.Module):
    ...
    def forward(self, x):
        ...
        dbox3 = dist2bbox(self.dfl3(box3), self.anchors.unsqueeze(0),
                          xywh=True, dim=1) * self.strides
    ...
    def __init__(self, cfg='yolo.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, (Detect, DDetect, Segment, Panoptic)):
            ...
            def forward(x): return self.forward(x)[0] if isinstance(
                m, (Segment, Panoptic)) else self.forward(x)
            m.stride = torch.tensor([s / x.shape[-2]
                                    for x in forward(torch.zeros(2, ch, s, s))])  # forward
        ...
        if isinstance(m, (DualDetect, TripleDetect, DualDDetect, TripleDDetect)):
            ...
            def forward(x): return self.forward(x)[0]
            m.stride = torch.tensor([s / x.shape[-2]
                                    for x in forward(torch.zeros(2, ch, s, s))])  # forward
            # check_anchor_order(m)
            # m.anchors /= m.stride.view(-1, 1, 1)
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
                ...
                RepNCSPELAN4, SPPELAN, DilatedReparamNCSPNCSPELAN4, RFCAConv, DCLGSeq}:
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]]
            if m in {BottleneckCSP, SPPCSPC, DCLGSeq}:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='./detect/yolov9-c-expb.yaml', help='model.yaml')
    # parser.add_argument('--cfg', type=str, default='./detect/gelan-c.yaml', help='model.yaml')
